...
static uint locPart1(char const* aFile)
{
	...
	GC_HashMap<uint, GC_HybridArray<state, 32>> queue;
	queue.Reserve(100000);
	queue.GetOrAdd(0).Add(initState);

	GC_HashSet<state> visited;
	...
	auto addTarget = [&](char& unit, char& tile, uint moves)
	{
		...
		uint const totalCost = cost + costs[unit] * moves;
		GC_Swap(unit, tile);
		queue.GetOrAdd(totalCost).Add(candidate);
		GC_Swap(unit, tile);
	};
